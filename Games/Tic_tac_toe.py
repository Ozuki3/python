...
def make_move(player, board):
    ...
    while True:
        try:
            move = int(input("Please enter a number 1-9:"))-1
            if move in range(len(board)):
                if board[move] == 0:
                    board[move] = player
                    break  
                else:
                    print('Board position is occupied, try another number: ') 
            else:
                print('Invalid range! Please enter a number 1-9：')
        except:
            print('Invalid input! Please enter an interger number 1-9：')
        
    


def check_win_horizontal(board):
    '''checks a board horizontally to see if there's a winner

    delegates to functions that check horizontally to see if player ID on a horizontal line of the board are the same. Returns the first winner
...
def check_win_vertical(board):
    '''checks a board vertically to see if there's a winner

    delegates to functions that check vertically to see if player ID on a vertical line of the board are the same. Returns the first winner
    found in the case of multiple winners.
...
def check_win_diagonal(board):
    '''checks a board diagonally to see if there's a winner

    delegates to functions that check diagonally to see if player ID on a diagonal line of the board are the same. Returns the first winner
    found in the case of multiple winners.
...
while(moves_left > 0 and winner == 0):
    ...
    moves_left -= 1
    print("Game over!",'Nobody' if winner == 0 else winner, "wins the game!")

...
def test_oauth():
    ...
    print(authentication_state.status_code)
...
def make_request(baseurl, params):
    ...
    dict
        ...
        a dictionary
    '''
    request = requests.get(baseurl, params=params, auth=oauth).json()
...
def make_request_with_cache(baseurl, hashtag, count):
    ...
    p_dict['q'] = hashtag
    p_dict['count'] = count
    request = construct_unique_key(baseurl, p_dict)
    if request in CACHE_DICT.keys():
        print("fetching cached data")
    else:
        print("making new request")
        CACHE_DICT[request] = make_request(baseurl, p_dict)
...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    # Using 'http://jsonviewer.stack.hu/' to see the json's structure
    hashtag = []
    value = []
    for i in tweet_data.values():
        value.append(i)
        statuses = value[0]
        for i in statuses:
            for j in i["entities"]["hashtags"]:
                if '#'+ j["text"].lower() != hashtag_to_ignore.lower():
                    hashtag.append(j['text'].lower())

    most_common_tag = max(hashtag, key=hashtag.count)
